...
def colorFilter(img):
    ...
    lowerTellow = np.array([18, 94, 140])
    upperYellow = np.array([48, 255, 255])
    lowerWhite = np.array([0, 0, 200])
    upperWhite = np.array([255, 255, 255])
    mashedWhite = cv2.inRange(hsv, lowerWhite, upperWhite)
    maskedYellow = cv2.inRange(hsv, lowerTellow, upperYellow)
    combinedImage = cv2.bitwise_or(mashedWhite, maskedYellow)
...
def initializeTrackers(initialTrackbarVals):
    cv2.namedWindow("Trackbars")
    cv2.resizeWindow("Trackbars", 360, 240)
    cv2.createTrackbar("Width Top", "Trackbars", initialTrackbarVals[0], 50, nothing)
    cv2.createTrackbar("Height Top", "Trackbars", initialTrackbarVals[1], 50, nothing)
    cv2.createTrackbar("Width Bottom", "Trackbars", initialTrackbarVals[2], 50, nothing)
    cv2.createTrackbar("Height Bottom", "Trackbars", initialTrackbarVals[3], 50, nothing)
